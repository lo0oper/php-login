<?php
    require '/opt/lampp/htdocs/dbconfig/config.php';
    ...
    if(isset($_POST['signup_btn'])){
        ...
        <?php
            if(isset($_POST['signup_btn'])){
                ...
                if($password==$cpassword){
                    $query ="select * from userinfo WHERE username='$uname'";//fetches the data from the table with uname as username
                    $query_run=mysqli_query($con,$query);//this function returns table(data from database ) same as that of query
                
                    if(mysqli_num_rows($con,$query) > 0)// numbber of rows returned 
                    {
